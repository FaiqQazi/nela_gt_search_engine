...
def Create_Forward_Index(input_directory,output_directory,output_prefix='forward_index', batch_size=10000):
    ...
    with open(input_directory, 'r') as inputFile:
        ...
        for article in articles:
            ...
            words.extend(title)
            if article['url'] not in doc_id_url.values():
                doc_id_url[doc_id]=article['url']
                doc_id+=1
                print("inserting in metadata")
            metadata_entry = {
            'url': article['url'],
            'title': article['title'],
            'date': article['date'],
...
out2=r'E:\salmandsa\DSA_Project'
# output prefix
...
# print(metadata)
# print(doc_id_url)
print("now working on the doc_id file")
output_file1 = os.path.join(out2, "metadata.json")
output_file2 = os.path.join(out2, "doc_id_url.json")
with open(output_file2, 'w') as doc_id_url_file:
    json.dump(doc_id_url, doc_id_url_file)

print("now working on the meta data file")

with open(output_file1, 'w') as metadata_list_file:
    json.dump(metadata, metadata_list_file)
